...
RUN echo "deb http://deb.debian.org/debian buster non-free" >> /etc/apt/sources.list && \
    ...
    #apt-get install --no-install-recommends -y openjdk-11-jdk git zip unzip bzip2 unrar-free curl wget && \
    #apt-get install -y openjdk-11-jdk git zip unzip bzip2 unrar-free curl wget && \
